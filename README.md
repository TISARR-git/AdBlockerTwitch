...
* 🔓 **Débloqueur de VODs (Sub-Only)** : Contourne les restrictions des chaînes pour vous permettre de regarder gratuitement les rediffusions réservées aux abonnés. *(Nécessite un backend proxy).*
* ⏪ **Lecteur DVR (Direct)** : Met en cache le direct pour vous permettre de mettre pause, de reculer et d'avancer dans le stream en live, exactement comme à la télévision.
